...
1. **졸업사정표 우선**: `get_graduation_status()`의 `graduationSummary.chapel.satisfied`(bool)로 판정한다.
   ...
   - `satisfied == False` → **포함 대상**. 이때 `requirements`에서 채플 총 요구 회수("채플 N회")를 뽑으면 안내에 **"충족 안 됨 + 총 N회 필요"**를 포함한다. 단 **"몇 회 남았다"는 금지** — 채플은 `takenCourses`에서 제외돼 이수 회수를 못 세 잔여(요구−이수) 계산이 불가하므로, "충족 안 됨 + 총 N회 필요" 수준만 말한다.
...
- **`profile.grade == 4` & `satisfied == False`**: 재수강 안내와 같은 뉘앙스로 강조한다 ("이번에 안 들으면 졸업이 걸려").
...
**충돌 처리** (자동 우선순위 없음 — 사용자에게 판단을 맡긴다):
- 채플 시간에 다른 과목이 겹치면 **자동으로 우선순위를 정하지 않는다**. 충돌 사실(요일·시간, 양쪽 과목)을 알려주고 선택지를 제시한다:
  - **(A) 다른 채플 분반** — `subject_groups` 기준으로 교체 후보 열거. 자기 대상 우선, 자기 대상 분반이 없으면 인터뷰 "타 대상 OK" 답 또는 사용자 확인 후에만 타 대상.
  - **(B) 겹치는 과목의 분반/대체 과목 변경**.
  - **(C) 채플 빼기** (잔여가 있어도 사용자 선택 시).
...
- 채플 분반 교체(충돌 시 다른 채플로 바꾼 경우)도 `conflicts_summary`에 남긴다.
