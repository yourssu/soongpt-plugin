...
**분반 열거**: 채플은 같은 `subject_key`(예: 비전채플 `code[:-2]`)의 분반 그룹. `subject_groups[subject_key]`로 전체 분반을 시간/교수/`target`과 함께 열거 — 분반 1개만 제시하지 않는다 (③ 재수강과 동일).
...
- 채플 시간에 다른 과목이 겹치면 **자동으로 하나만 고르지 않는다**. 충돌 사실(요일·시간, 양쪽 과목)을 알리고, 아래 세 방식을 **각각 적용한 후보 변형**을 만들어 함께 제시한다:
  ...
  - **(C) 채플 빼기** (잔여가 있어도).
- 사용자가 후보 변형을 고르면 그것을 확정 후보로 이어간다.
- 분반 선택은 **`subject_groups`(`subject_key`) 기준**을 유지한다.
