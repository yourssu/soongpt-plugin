...
- **학점 합산 제외**: 채플 `credits`(보통 0.5)는 **`total_credits`에서 제외** (학기당 학점 상한에 안 걸림 — 학사규정). 후보 표시에 "채플 1과목(총학점 제외)"으로 따로 표기.
...
- **Case A — `(대상외수강제한)` 표기 있음** → **"대상외 수강신청"** 대상. 정규 수강신청이 아니라 **별도의 '대상외 수강신청' 기간**(학기 초에 짧게 열림)에 담아야 한다. 후보에서 **제외**하거나 **명시적으로 경고**한다.
...
- 후보는 **A/B/C** 형태로 제시한다:
  ...
  - 장단점 (충돌 유무, **`대상외수강제한` 표기 과목 포함 여부**, 교수 선택 폭 등)
...
- 사용자가 후보를 **확정**하면 `save_timetable_candidate(year, semester, candidate, generation_params)`를 호출한다.
  - `candidate` 필드:
    ...
    - `total_credits`: 학점 합계 (float) — **채플 학점(0.5)은 제외** (학사규정: 채플은 학기 학점 상한에 안 걸림)
