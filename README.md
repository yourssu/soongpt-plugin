...
| `find_lectures` | 특정 학기/카테고리 강의 검색 (강의계획서 옵션). 기본 `save_to_cache=True`로 결과를 서버 측에서 캐시에 자동 그룹 저장 (SPR-75). `summary=True`면 lectures 상세 생략 (SPR-76) — 단 `find_by_lecture`/`find_by_professor`는 summary 무시하고 항상 상세 (SPR-110) | 일반 카테고리 ~3초 / 교양선택 "전체"는 수초~30초대 |
...
**메인테이너 워크플로우** (학기별, 학기 시작 전): seed 파일이 없거나 stale 할 때 신규 사용자 첫 호출이 10~20초 걸리므로, 메인테이너가 직접 갱신해 커밋합니다. **캐시/seed의 semester가 현재 학기와 다르면 무시하고 자동 빌드한다** (SPR-100).
