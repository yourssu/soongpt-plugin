...
```
1. 처음: "내 프로필 설정해줘"
   → refresh_user_profile() 호출 → USAINT에서 3개 필드 추출 저장
...
2. 휴학/복학/전과 후: "내 프로필 업데이트해줘"
   ...
   → USAINT 3개 필드 갱신, 사용자가 입력한 학번/이름 등은 보존
...
`get_available_lectures(year, semester)`는 저장된 프로필 기반으로 여러 카테고리(전공/교양/채플 등)를 한 번에 병렬 조회합니다. `find_lectures`를 여러 번 호출할 필요 없이 한 번에 그룹화된 결과를 받습니다.

**현재 상태**: 뼈대(skeleton)만 구현되어 있습니다. 프로필이 없으면 RuntimeError, 있더라도 `build_category_requests()`가 빈 리스트를 반환하므로 `groups={}`가 돌아옵니다. 어떤 카테고리를 부를지 설정하는 스펙은 후속 PR에서 추가됩니다.
