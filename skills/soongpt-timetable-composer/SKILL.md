...
1. **졸업사정표 우선**: `get_graduation_status()`의 `graduationSummary.chapel.satisfied`(bool)로 판정한다.
   ...
   - `satisfied == False` → 잔여 있음, **포함 대상**. (정확한 잔여 회수는 `satisfied`에 없다 — `requirements`에 "채플 N회"로 보이면 표시용으로만 쓰고, **포함 여부는 `satisfied`로 결정**한다.)
...
**포함 기준**:
- 잔여 ≥ 1이면 **학년 무관 기본 포함**. "채플 빼기"는 사용자가 명시할 때만.
- **4학년 & 잔여 ≥ 1**: 재수강 안내와 같은 뉘앙스로 강조한다 ("이번에 안 들으면 졸업이 걸려").
...
**충돌 처리**:
- 채플 시간은 고정형 → 채플 자리는 유지 (잔여가 있는 한 빼지 않는다).
- 채플 시간에 다른 과목이 겹치면: 그 채플이 "무조건 들어야 하는 특정 분반"이 **아니면**, **다른 채플 분반으로 교체**해 충돌 회피 (채플 자체를 빼지 않음). 교체 후보도 **자기 대상 우선**이고, 자기 대상 분반이 없으면 인터뷰 "타 대상 OK" 답 또는 사용자 확인 후에만 타 대상으로 교체한다.
- **교체 불가**(분반이 1개뿐이어서 교체할 채플이 없거나, 사용자가 특정 분반 고정 지정) → 그 채플을 고정축으로 두고 겹치는 **다른 과목**을 다른 분반으로 옮긴다 (이동 불가 시 사용자 확인).
