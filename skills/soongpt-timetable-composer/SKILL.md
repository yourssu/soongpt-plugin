...
3. **교양선택(교선)** (Flex 후보 필요 시): `parse_lectures_cache(year, semester, category_prefixes=["교선"], include_subject_groups=False, entered_year=<profile.entered_year>)` 호출 — 서버가 **응답 전 field_tags 학번 필터**를 적용해 내 학번에 열린 교선만 **컴팩트 후보**(`code`/`name`/`credits`/`field_tags` 매칭 줄만 — **슬롯 없음**)로 반환한다 (SPR-99). 교선 `optional_elective_all`은 단일 그룹(~337강의)이라 전부 다 보면 parsed 상세 ~300KB → **50K 파일 스필**이 난다. **전부를 능동 추론에 올리지 말고** 이 컴팩트 후보로 Flex 교선 후보 풀을 좁힌 뒤, 실제 후보로 쓸 강의는 `parse_lectures_cache(year, semester, codes=[...])`로 **full 상세(슬롯 포함)를 그때 조회**해 충돌 검사에 쓴다 — `codes`는 한 번에 **~40개 이하**로 (초과 시 분할, 50K 한도). 컴팩트 항목(`slots` 없음)은 `check_timetable_conflicts`에 그대로 넘기지 말 것 (ValueError). `entered_year`는 진입 절차의 `get_user_profile`/snapshot에서 읽는다. **여기도 `include_subject_groups=False`** (2번과 동일 — 1번에서 이미 인덱스 확보).
