...
- `parsed`에서 `category`가 `"전기-"`/`"전필-"`로 시작하거나 `"교필"`인 강의 중, `target` 자연어를 해석(LLM)해 수강 가능한 것만 후보로 제시한다. 단 **교양필수 충족 시(SPR-91) `교필`은 위 파싱 2번에서 제외됐으므로 parsed에 아예 없다** — 이미 충족했으니 안내·후보에서 빠지는 게 맞다 (교필 재수강은 ③의 find_by_lecture 폴백이 잡는다).
  ...
  - **Case A 경고 (대상외수강제한)**: 제시하는 강의의 `target`에 `(대상외수강제한)` 문자열이 있으면 정규 수강신청이 아니라 **별도의 '대상외 수강신청' 기간**(학기 초에 짧게 열림)에 담아야 한다고 **명시적으로 경고**하고 후보에는 유지한다. 전필 등 졸업 필수 과목이면 제외하면 이수 누락이 생기므로 "제외 또는 명시 경고" 중 **명시 경고**를 택한다:
...
- 후보는 **A/B/C** 형태로 제시한다:
  ...
  - 장단점 (충돌 유무, 교수 선택 폭 등)
  - **대상외 수강신청 필요 과목 (Case A — 필수 표기)**: 후보에 `(대상외수강제한)` 표기 과목이 있으면 **빠짐없이 나열**한다. 후보별로 다르면 후보마다 따로 표기하고, [수강제한 규칙](#수강제한-규칙) Case A 문구("별도 '대상외 수강신청' 기간에 담아야 해")로 안내한다. 예:
...
- 사용자가 후보를 **확정**하면 `save_timetable_candidate(year, semester, candidate, generation_params)`를 호출한다.
  - `candidate` 필드:
    ...
    - `conflicts_summary`: 충돌/불확정/empty warnings 요약 — **`check_timetable_conflicts`의 `warnings` 포함 필수**. **Case A `(대상외수강제한)` 표기 과목 목록도 반드시 포함**한다 — 후보 확정 후(시각화/재개)에도 "대상외 수강신청 기간 필요"가 유지되도록:
      ```
