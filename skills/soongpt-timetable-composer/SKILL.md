...
- **포함 결정 — Base 기본 포함**: grade==1이면 소그룹채플이 **필수** → 자기 대상 분반을 **`satisfied` 여부와 무관하게 Base에 기본 포함** (사용자가 명시적 제외/escape 하지 않는 한).
  - **`satisfied`는 안내용만**: 1학년은 사실상 `graduationStatus.chapel.satisfied == False`(6회 미충족). 포함은 "grade==1 + 자기 대상 분반 존재"로 결정한다. 예외적으로 `satisfied == True`면 "이번엔 안 들어도 졸업엔 문제 없어"만 덧붙이되, 1학년 정책상 여전히 기본 포함.
